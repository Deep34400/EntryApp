/**
 * StaffPurposeScreen — Select staff purpose from PURPOSE_CONFIG (dp_type === "staff").
 * No driver lookup; submits with entryType "non_dp".
 */
...
type NavProp = NativeStackNavigationProp<RootStackParamList, "StaffPurpose">;
type RoutePropType = RouteProp<RootStackParamList, "StaffPurpose">;
...
export default function StaffPurposeScreen() {
  ...
  const items = staffCategory?.items ?? [];
  ...
  const submitMutation = useMutation({
    mutationFn: async (subCategory: string) => {
      ...
      const body = buildEntryPayload({
        formData,
        category: "Staff" as VisitorCategory,
        subCategory,
        entryType: "non_dp",
      });
    ...
    onSuccess: (data) => {
      ...
      const apiType = raw.type ?? raw.entry_type ?? "non_dp";
      navigation.navigate("TokenDisplay", {
        ...
        entryType: apiType != null ? String(apiType) : "non_dp",
    ...
    >
      ...
      >
        ...
        {(configLoading || configRefetching) && !purposeConfig ? (
          ...
          <View style={styles.errorState}>
            <ThemedText type="small" style={styles.errorStateText}>
              No staff purposes configured. Please try again later.
            </ThemedText>
          </View>
        ) : (
