...
import {
  ...
  Image, KeyboardAvoidingView, Platform, Keyboard, Animated, Easing,
  RefreshControl,
} from "react-native";
...
function FormInput({
  label, prefix, ...props
}: React.ComponentProps<typeof TextInput> & { label: string; prefix?: string }) {
            ...
            >
              ...
              {selectedReg.length > 0 && (
                <>
                  ...
                  <Pressable
                    onPress={() => { onClear(); setQuery(""); closeDrop(); inputRef.current?.blur(); }}
...
export default function VisitorTypeScreen() {
  ...
  const fetchByPhone = async () => {
    ...
    try {
      ...
      if (d) {
        setDriver(d);
        setForm((p) => ({ ...p, name: d.name, vehicle_reg_number: p.vehicle_reg_number || "" }));
      }
  ...
  const fetchByReg = async () => {
    ...
    try {
      ...
      if (d) {
        setDriver(d);
        setForm((p) => ({ ...p, name: d.name, phone: normalizePhoneInput(d.phone) }));
      }
    } finally { setLoading(false); }
  };
